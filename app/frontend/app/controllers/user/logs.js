...
export default Controller.extend({
  ...
  actions: {
    ...
    update_logging_code: function() {
      ...
      LingoLinqAAC.session.set('logging_codes', codes);
      this.send('refresh');
    },
    refresh: function() {
    ...
    generate: function() {
      ...
      modal.open('modals/manual-log', {external_device: !!_this.get('model.external_device')}).then(function(res) {
        if(res && res.words && res.words.length > 0 && res.date) {
          var file = LingoLinqAAC.Log.generate_obf(res.words, res.date);
    ...
    import: function(file) {
      ...
      LingoLinqAAC.Log.import(file, log_type, user_id).then(function(logs) {
        if(logs.length == 1) {
