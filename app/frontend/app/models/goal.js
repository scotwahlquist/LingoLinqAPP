...
LingoLinqAAC.Goal = DS.Model.extend({
  didLoad: function() {
    this.check_badges();
  ...
  generate_next_template_if_new: function() {
    if(this.get('new_next_template_id')) {
      var next_template = LingoLinqAAC.store.createRecord('goal');
  ...
  current_template: computed('currently_running_template', function() {
    if(this.get('currently_running_template')) {
      return LingoLinqAAC.store.createRecord('goal', this.get('currently_running_template'));
    } else {
      return this;
    }
  }),
...
export default LingoLinqAAC.Goal;
